...
<div class="section">
            <div class="section-header">
                ...
                <div class="section-header-back">
                    <a href="/petugas/pangkat/create" class="btn btn-icon btn-primary" title="Tambah Pangkat">
                ...
                <div class="section-header-breadcrumb">
                   ...
                   @endphp
                    ...
                    <div class="breadcrumb-item  {{Request::is($link.'pangkat')  ? 'active' : ''}}">
                        Pangkat
                    </div>
            ...
            <div class="section-body">
                <div class="card">
                  <div class="card-header">
                    ...
                    <div class="card-header-action">
                        @if (Auth::user()->level == 'Petugas')
                            <form action="/petugas/pangkat" method="get">
                        @elseif(Auth::user()->level == 'Pimpinan')
                            <form action="/petugas/pangkat" method="get">
                                @else
                        @endif
                ...
                <div class="card-body row col-md-12">
                             ...
                             <td>
                                @if(Auth::user()->level == 'Petugas')
                                        <div class="form-group row mt-3">
                                            ...
                                            <a href="/petugas/pangkat/{{ $row->id }}/edit" class="btn btn-warning btn-sm"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                            <form action="/petugas/pangkat/{{ $row->id}}" method="post" class="align-content-center" id="del-{{ $row->id}}">
                                           @csrf
                                        @method('delete')
                                          <button class="btn btn-danger btn-sm mt-1 ml-3" data-confirm="Hapus Data?|Apakah Anda yakin Ingin Menghapus Data <br> Nama : {{ $row->pegawai->nama }} <br> Pangkat : {{ $row->pangkat }} ?" data-confirm-yes="submitDel({{ $row->id}})">
                           ...
                           <div class="modal fade" tabindex="-1" role="dialog" id="ModalUp{{ $row->id }}" aria-hidden="true" style="display: none;">
                                              <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                  ...
                                                  <div class="modal-body">
                                                    <form action="/petugas/pangkat/{{ $row->id }}/upload" method="POST" enctype="multipart/form-data" class="modal-upload-form">
                                                        @csrf
                                                        @method('put')
                  ...
                  <div class="card-footer text-right">
                    <div class="text-left">
                   <i> Showing {{ $pangkat->firstItem() }} to {{ $pangkat->lastItem() }} of {{ $pangkat->total() }} Entires</i>
                  </div>
                  <nav class="pull-right d-lg-inline-block">
                   <ul class="pagination mb-0">
                       <li class="page-item{{ ($pangkat->onFirstPage()) ? ' disabled' : '' }}">
                           <a class="page-link" href="{{ $pangkat->previousPageUrl() }}"><i class="fas fa-chevron-left"></i></a>
                       </li>

                       @if ($pangkat->lastPage() > 1)
                           @for ($i = 1; $i <= $pangkat->lastPage(); $i++)
                               <li class="page-item{{ ($pangkat->currentPage() === $i) ? ' active' : '' }}">
                                   <a class="page-link" href="{{ $pangkat->url($i) }}">{{ $i }}</a>
                               </li>
                           @endfor
                       @endif

                       <li class="page-item{{ ($pangkat->hasMorePages()) ? '' : ' disabled' }}">
                           <a class="page-link" href="{{ $pangkat->nextPageUrl() }}"><i class="fas fa-chevron-right"></i></a>
                       </li>
                   </ul>
                 </nav>
                        </div>
