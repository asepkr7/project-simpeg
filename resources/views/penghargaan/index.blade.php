...
<div class="section">
            <div class="section-header">
                ...
                <div class="section-header-back">
                    <a href="/petugas/penghargaan/create" class="btn btn-icon btn-primary" title="Tambah {{ $title }}">
                ...
                <div class="section-header-breadcrumb">
                   ...
                   @endphp
                    ...
                    <div class="breadcrumb-item  {{Request::is($link.'penghargaan')  ? 'active' : ''}}">
                        Penghargaan
                    </div>
            ...
            <div class="section-body">
                <div class="card">
                  <div class="card-header">
                    ...
                    <div class="card-header-action">
                        @if (Auth::user()->level == 'Petugas')
                            <form action="/petugas/penghargaan" method="get">
                        @elseif(Auth::user()->level == 'Pimpinan')
                            <form action="/pimpinan/penghargaan" method="get">
                                @else
                                <form action="/pegawai/penghargaan" method="get">
                        @endif
                ...
                <div class="card-body row lg-12">
                             ...
                             <td>
                                ...
                                @if(Auth::user()->level == 'Petugas')
                                           <button class="btn btn-sm btn-primary mr-1" data-toggle="modal" data-target="#ModalUp{{ $row->id }}" ><i class="fas fa-upload"></i></button>
                                             <a href="/petugas/penghargaan/{{ $row->id }}/edit" class="btn btn-warning btn-sm"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                             <form action="/petugas/penghargaan/{{ $row->id}}" method="post" class="align-content-center" id="del-{{ $row->id}}">
                                                @csrf
                                                @method('delete')
                                                <button class="btn btn-danger btn-sm mt-1 ml-3" data-confirm="Hapus Data?|Apakah Anda yakin Ingin Menghapus Data <br> Nama : {{ $row->pegawai->nama }} <br> Penghargaan : {{ $row->penghargaan }} ?" data-confirm-yes="submitDel({{ $row->id}})">
                          ...
                          <div class="modal fade" tabindex="-1" role="dialog" id="ModalUp{{ $row->id }}" aria-hidden="true" style="display: none;">
                                              <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                  ...
                                                  <div class="modal-body">
                                                    <form action="/petugas/penghargaan/{{ $row->id }}/upload" method="POST" enctype="multipart/form-data" class="modal-upload-form">
                                                        @csrf
                                                        @method('put')
                  ...
                  <div class="card-footer text-right">
                    <div class="text-left">
                   <i> Showing {{ $penghargaan->firstItem() }} to {{ $penghargaan->lastItem() }} of {{ $penghargaan->total() }} Entires</i>
                  </div>
                  <nav class="pull-right d-lg-inline-block">
                   <ul class="pagination mb-0">
                       <li class="page-item{{ ($penghargaan->onFirstPage()) ? ' disabled' : '' }}">
                           <a class="page-link" href="{{ $penghargaan->previousPageUrl() }}"><i class="fas fa-chevron-left"></i></a>
                       </li>

                       @if ($penghargaan->lastPage() > 1)
                           @for ($i = 1; $i <= $penghargaan->lastPage(); $i++)
                               <li class="page-item{{ ($penghargaan->currentPage() === $i) ? ' active' : '' }}">
                                   <a class="page-link" href="{{ $penghargaan->url($i) }}">{{ $i }}</a>
                               </li>
                           @endfor
                       @endif

                       <li class="page-item{{ ($penghargaan->hasMorePages()) ? '' : ' disabled' }}">
                           <a class="page-link" href="{{ $penghargaan->nextPageUrl() }}"><i class="fas fa-chevron-right"></i></a>
                       </li>
                   </ul>
                 </nav>
                        </div>
